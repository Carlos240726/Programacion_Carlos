...
public class OperacionesPath {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Path unPath = Path.of("/media/cargarrom7/Ventoy/");
        Path otroPath = Paths.get("/media/cargarrom7/Ventoy/");

        // Obtener el nombre de un fichero y unirlo a la ruta de otro directorio
        Path unFile = Path.of("/media/cargarrom7/Ventoy/file.txt");
        Path unDir = Path.of("/media/cargarrom7/Ventoy/");
        // /home/user/file.txt
        System.out.println(unDir.resolve(unFile.getFileName()));

        // Relativizar una ruta respecto a otra
        Path otroFile = Path.of("/media/cargarrom7/Ventoy/Javascript/file.txt \n");
        Path otroDir = Path.of("/media/cargarrom7/Ventoy/Javascript/");
        // obtener c/d.txt
        ...
        Files.createDirectories(Path.of("/media/cargarrom7/Ventoy/me/voy/a/suicidar/"));

        System.out.println("\n siguiente \n");
        // Imprimir el contenido de un directorio
        Files.list(Path.of("/media/cargarrom7/Ventoy/Carpeta nova/")).forEach(System.out::println);


        // Obtener la lista de los contenidos anidados de un directorio
        List<Path> list = Files.walk(Paths.get("/media/cargarrom7/Ventoy/Carpeta nova/")).collect(Collectors.toList());
        System.out.println("\n siguiente \n");

        // Imprimir los ficheros regulares de un directorio ordenados por tamaño
        Files.list(Path.of("/media/cargarrom7/Ventoy/"))
        ...
        Path directorio = Path.of("/media/cargarrom7/Ventoy/");
        ...
        Path file = Path.of("/media/cargarrom7/Ventoy/LLM/");
        ...
        Path original = Path.of("/media/cargarrom7/Ventoy/prueba.iml");
        Path backup = Path.of("/media/cargarrom7/Ventoy/prueba.iml.backup");
        Path rename = Path.of("/media/cargarrom7/Ventoy/prueba.iml.backup.1");
